...
class PoetryDjangoAppTestCase(BaseTestCase):
    def test_basic(self):
        with self.assertLogs('cookiecutter', level=logging.DEBUG) as logs:
            pkg_path: Path = run_cookiecutter(
                ...
                final_name='your_cool_django_app',  # {{ cookiecutter.package_name }} replaced!
                # force_recreate=True
        ...
        if not Path(pkg_path / '.venv' / 'bin' / 'your_cool_django_app').is_file():
