...
def install():
    """
    Run pip-sync and install '{{ cookiecutter.ynh_app_pkg_name }}' via pip as editable.
    """
...
def publish():
    ...
    publish_package(
        module={{ cookiecutter.ynh_app_pkg_name }},
        package_path=PACKAGE_ROOT,
        distribution_name='{{ cookiecutter.ynh_app_pkg_name }}',
    )
...
def fix_code_style(color: bool = True, verbose: bool = False):
    """
    Fix code style of all {{ cookiecutter.ynh_app_pkg_name }} source code files via darker
    """
...
def main():
    print_version({{ cookiecutter.ynh_app_pkg_name }})
